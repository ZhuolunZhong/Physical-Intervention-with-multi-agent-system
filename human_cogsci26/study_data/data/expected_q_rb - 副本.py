    ...
    for ax, (rnds, title, st) in zip(axes, settings):
        ...
        if not red_df.empty:
            if st <= 2:
                ...
                if x_sm.size:
                    # 计算置信区间
                    x_ci, y_ci, (lower_ci, upper_ci) = bootstrap_ci(r['time'].values, r['ExpectedQvalue'].values)
                    if x_ci.size:
                        # 对置信区间也做差值处理
        ...
        if not blue_df.empty:
            if st <= 2:
                ...
                if x_sm.size:
                    # 计算置信区间
                    x_ci, y_ci, (lower_ci, upper_ci) = bootstrap_ci(b['time'].values, b['ExpectedQvalue'].values)
                    if x_ci.size:
                        # 对置信区间也做差值处理
                        base_x_all, base_y_all = base_lines.get('all', (np.array([]), np.array([])))
                        if base_x_all.size:
